#ifndef UAV_SAR_P1_SENSING_MODEL_H
#define UAV_SAR_P1_SENSING_MODEL_H

// The node sensing model, and what Phase 1 PRODUCES.
//
// ---------------------------------------------------------------------------
// WHERE THIS SITS IN THE PIPELINE
// ---------------------------------------------------------------------------
//   nodes -> cells + cluster heads -> the CH set is the waypoint set
//         -> two scheduling variants -> rough tours -> refinement
//         -> THE AIRCRAFT FLIES AND BROADCASTS
//         -> only THEN is there a set of suspect positions
//
// The suspect set is an OUTPUT, not an input. Nothing here may be fed back into
// the planner: at planning time no node holds the reference, so no node can say
// what is there, and a planner handed a suspect set would be using information
// the flight has not produced yet.
//
// ---------------------------------------------------------------------------
// THE ONE THING THIS FILE MUST GET RIGHT
// ---------------------------------------------------------------------------
// The SAME node, with the SAME noise draw, has TWO readings:
//
...
//   scoreFull   what THAT SAME node, with THAT SAME noise, reads holding the
//               complete reference. The confuser stops matching; a real victim
//               does not.
//
// Both come out of one observation, because they are one observation. Drawing
// them separately would make the post-delivery reading an unrelated second coin
// flip, and the claim the whole architecture rests on -- that DELIVERING IS AN
// ACT OF DISAMBIGUATION, not an act of transport -- would quietly stop being
// modelled.
//
// The gap between the two readings is exactly what the aircraft flies out to
// buy. Before delivery a node is bounded by Fano at P(correct) <= 1/(M+1) over
...
// does not move it, and only information from outside does.
//
// scoreCue is therefore NOT a planning input in this design. It is the BASELINE
// the flight is measured against: what the network could have said on its own,
// against what it can say once the reference has arrived. That comparison is a
// result, and it is the only thing scoreCue is for.
//
...
struct SensingResult {
    ...
    // What the network WOULD have said with no reference at all. Kept as the
    // baseline the flight is measured against -- never fed to the planner.
    std::vector<int32_t> cueGuess;          // best cue-level score first
...
// Realise the sensing: one noise draw per node, two readings each. Depends only
// on the world and the deployment -- never on the plan.
SensingResult RunSensing(const std::vector<Node>& nodes, const CellPlan& plan,
                         const std::vector<Object>& objects, uint32_t seed);

// What a cell concludes once it holds the reference -- the step that PRODUCES a
// suspect position. Uses the SAME reading realisation, which is the point.
...
#endif  // UAV_SAR_P1_SENSING_MODEL_H
