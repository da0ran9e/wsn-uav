...
| `BAO-CAO-TIEN-DO-vi.md` | Vietnamese: **phase progress report** — every algorithm, model and heuristic implemented in this phase, each explained in plain terms with its formula: two-tier sensing, noisy-OR cell aggregation, evidence-weighted election, spatial stand-down, distributed task assignment, fixed-wing lane planning and turn kinematics, the courier and sweep-gate policies, the multi-fix report, plus the P10 formulation, the measurement definitions, and what is still open. Start here for "what was built and why" |
