/*
 * Scenario 2: Multi UAV (2 UAVs), Grid-based Ground Network
 *
 * Simulates 2 UAVs disseminating K fragments over an N×N grid of ground nodes.
 *
 * Usage:
 *   ./ns3 run scenario-2-multi-uav-2 -- --gridSize=15 --numFragments=10 --seed=1 --runId=1
 *
 * Output:
 *   data/results/scenario-2/run-001/
...
NS_LOG_COMPONENT_DEFINE("Scenario2MultiUav2");
...
main(int argc, char* argv[])
{
    ...
    config.numUavs = 3; // Default to 3 UAVs
    ...
    if (config.outputDir == "src/wsn-uav/results/scenario-1/run-001")
    {
        ...
        oss << "src/wsn-uav/results/scenario-2/run-" << std::setfill('0') << std::setw(3) << config.runId;
    ...
    // =========================================================================
    LogComponentEnable("Scenario2MultiUav2", LOG_LEVEL_INFO);
    ...
    NS_LOG_INFO("=== Scenario 2: Multi-UAV (2) Fragment Dissemination ===");
